...
/ip hotspot profile add name=hotspotradius login-by=http-chap use-radius=yes
/ip hotspot add name=hotspot1 interface=bridge address-pool=hotspot profile=hotspotradius
/ppp profile add name=pppoeradius local-address=192.168.88.1 remote-address=pppoe
/interface pppoe-server server add service-name=isp-pppoe interface=bridge default-profile=pppoeradius
/queue type add name=pcqdownload kind=pcq
/queue type add name=pcqupload kind=pcq
